...
import { createStore, detectGitRepo } from './shared.js';
...
export const reviewCommand = new Command('review')
  .description('Fetch, auto-split, and launch web UI for a review (shortcut)')
  .argument('[pr-ref]', 'PR reference: org/repo#N or GitHub URL')
  .option('-b, --branch <branch>', 'Review a local branch (compare against default branch)')
  .option('-d, --diff <path>', 'Review a local patch/diff file')
  ...
  .action(
    async (
      ...
      try {
        // ─── Step 1: Fetch ──────────────────────────────────────────────────────
        // Precedence: --diff > --branch > prRef > error
        let reviewKey: string;
        let parsed: ReturnType<typeof parsePRReference> | undefined;

        if (options.diff) {
          // Local diff file mode
          const fetchResult = await fetchLocalDiff(store, options.diff);
          reviewKey = fetchResult.reviewKey;
        } else if (options.branch) {
          // Local branch mode — delete stale reviews matching this branch
          const allReviews = store.listReviews();
          for (const r of allReviews) {
            if (r.provider === 'local' && r.head === options.branch) {
              await store.deleteReview(r.key);
            }
          }
          const fetchResult = await fetchLocalBranch(store, options.branch);
          reviewKey = fetchResult.reviewKey;
        } else if (prRef) {
          // GitHub PR mode
          try {
          ...
          console.error(
            'Error: Please provide a PR reference, --branch, or --diff flag.\n' +
              'Usage:\n' +
              '  kaiju review org/repo#N\n' +
              '  kaiju review https://github.com/org/repo/pull/N\n' +
              '  kaiju review --branch feature-branch\n' +
              '  kaiju review --diff changes.patch',
          );
          process.exitCode = 1;
          return;
        }
        ...
        const allCommentsFetch = store.getComments(reviewKey);
        ...
        const fetchSummary: FetchSummaryData = {
          ...
          totalAdditions,
          totalDeletions,
          commentCount: allCommentsFetch.length,
          reviewDir,
        };
        ...
        // Context detection: use parsed PR owner/repo or fall back to git remote
        if (parsed) {
        ...
        // Construct the concrete review URL — use reviewKey for local sources, prRef for GitHub
        const reviewUrl = buildReviewUrl(url, parsed ? prRef : reviewKey);
